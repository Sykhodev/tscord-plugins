...
@injectable()
@Category('Admin')
@SlashGroup({ name: 'stars', localizationSource: 'StarBoard.STARBOARD.STAR_DESC' as TranslationsNestedPaths })
@SlashGroup('stars')
export default class StarsCommand {
	...
	@Slash({
		name: "set",
		localizationSource: 'StarBoard.STARBOARD.STAR_DESC' as TranslationsNestedPaths,
	})
	@Guard(
	...
	async set(
		@SlashOption({
			name: "channel",
			localizationSource: 'StarBoard.STARBOARD.STAR_SET_CHAN' as TranslationsNestedPaths,
			required: false,
		...
		@SlashOption({
			name: "emoji",
			localizationSource: 'StarBoard.STARBOARD.STAR_SET_EMOJI' as TranslationsNestedPaths,
			required: false,
		...
		@SlashOption({
			name: "minstar",
			localizationSource: 'StarBoard.STARBOARD.STAR_MIN_EMOJI' as TranslationsNestedPaths,
			required: false,
		...
		// Verifications
		if (!interaction.guildId) return interaction.followUp({ content: localize.STARBOARD.STAR_ERR_GUILD.MESSAGE(), ephemeral: true });
		...
		if (!chanStars) {
			...
			if (previousMessages && previousMessages.length > 0) {
				previousMessages.forEach(el => wrap(el).assign({ starboardChannel: chanStars.channelId }));
				await starMessage.getEntityManager().persistAndFlush(previousMessages);
			}
